	...
	for (size_t y = 0; y < getHeight(); y++)
	{
		if (_table[y] == "")
			for (size_t x = 0; x < getWidth(); x++)
				_table[y].push_back(' ');
		else if (y == legendCoord.getY() || y == static_cast<int>(legendCoord.getY()) + 1 || y == static_cast<int>(legendCoord.getY()) + 2)
			for (int x = legendCoord.getX(); x < legendCoord.getX() + static_cast<int>(boardConstants::LEGEND_MAX_WIDTH); x++)
				if (isWall(x, static_cast<int>(y)) == true)
...
void Board::Print(bool colorAllowed, const string& screenName) const
{
	...
	{
		for (size_t x = 0; x < getWidth(); x++)
			if (_table[y][x] == static_cast<char>(boardConstants::LEGEND))
			...
			else
			{
				if (colorAllowed == true)
					if (_table[y][x] == static_cast<char>(boardConstants::EXIT))
						...
						SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), static_cast<int>(boardConstants::WALL_COLOR));
				std::cout << _table[y][x];
			}
		std::cout << std::endl;
	}
